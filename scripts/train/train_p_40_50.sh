...
do
    for P in "40" "50"
    do
        echo "Running model $MODEL_DIR for p = $P, k = $K" >> $LOG_FILE
        start=$SECONDS

        K_P_CONFIG_PATH=$CONFIG_PATH"_"$P"/dropout_"$P"_k_"$K".yml"
        ./scripts/train/train.sh --datasets_dir $DATASETS_DIR --model_dir $MODEL_DIR --config_path $K_P_CONFIG_PATH --output_dir $OUTPUT_DIR --log_file $LOG_FILE
