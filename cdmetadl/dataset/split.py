__all__ = ["split_by_names", "random_meta_split", "random_class_split", "rand_conf_split"]
...
def rand_conf_split(
    support_set: tuple[torch.Tensor, torch.Tensor, torch.Tensor], query_set: tuple[torch.Tensor, torch.Tensor,
                                                                                   torch.Tensor], num_ways: int,
    num_shots: int, nr_splits_support: int = 3, seed: int = None
):
    """
    ...
    indices_per_way_query = [
        np.array_split(random.sample(list(np.arange(query_size)), query_size), nr_splits_query)
        for cls in range(num_ways)
    ]

    indices_per_way_support = [
        np.array_split(random.sample(list(np.arange(num_shots)), num_shots), nr_splits_support)
        for cls in range(num_ways)
    ]

    # make list of images for putting into tensor
    # num_splits x num_ways*num_shots_per_split
    split_support_images = [[
        support_images_by_shots[i][j] for j, indices in enumerate(indices_per_way_support) for i in indices[split]
    ] for split in range(nr_splits_support)]
    split_query_images = [[
        query_images_by_query_size[i][j] for j, indices in enumerate(indices_per_way_query) for i in indices[split]
    ] for split in range(nr_splits_query)]
    ...
    split_support = [(
        torch.tensor(np.array(split_support_images[i])), torch.tensor(np.arange(num_ways).repeat(support_shots_nr[i])),
        torch.tensor(original_labels.repeat(support_shots_nr[i]))
    ) for i in range(nr_splits_support)]

    split_query = [(
        torch.tensor(np.array(split_query_images[i])), torch.tensor(np.arange(num_ways).repeat(query_shots_nr[i])),
        torch.tensor(original_labels.repeat(query_shots_nr[i]))
    ) for i in range(nr_splits_query)]
