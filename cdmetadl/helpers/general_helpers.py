...
def exist_dir(dir: pathlib.Path) -> bool:
    """ Check if a directory exists.

    Args:
        dir (pathlib.Path): Directory to be checked.
...
def exist_file(file: pathlib.Path) -> bool:
    """ Check if a file exists.

    Args:
        file (pathlib.Path): File to be checked.
...
def load_yaml(file: str) -> dict:
    ...
    try:
        ...
        raise OSError(f"In load_yaml, file '{file}' could not be opened or " + f"has wrong format")
...
def load_json(file: str) -> dict:
    ...
    try:
        ...
        raise OSError(f"In load_json, file '{file}' could not be opened or " + f"has wrong format")


def check_datasets(input_dir: pathlib.Path, datasets: List[str], verbose: bool = False) -> dict:
    ...
    Args:
        input_dir (pathlib.Path): Path to the input data directory.
    ...
    for i, dataset in enumerate(datasets):
        ...
        JSON_PATH = dataset_path / "info.json"
        ...
        if "image_column_name" in info:
            ...
            raise Exception("In check_datasets, 'image_column_name' not found " + f"in '{JSON_PATH}'")
        if "category_column_name" in info:
            CATEGORY_COLUMN = info["category_column_name"]
        else:
            vprint("\t\t\t[-] JSON file", verbose)
            raise Exception("In check_datasets, 'category_column_name' not " + f"found in '{JSON_PATH}'")
        vprint("\t\t\t[+] JSON file", verbose)
        ...
        try:
            ...
            raise OSError(f"In check_datasets, file '{CSV_PATH}' could not be " + f"opened or has wrong format.")
        ...
        if IMAGE_COLUMN not in csv_columns:
            vprint("\t\t\t[-] CSV file", verbose)
            raise Exception(f"In check_datasets, column '{IMAGE_COLUMN}' " + f"not found in '{CSV_PATH}'")

        # Category
        if CATEGORY_COLUMN not in csv_columns:
            vprint("\t\t\t[-] CSV file", verbose)
            raise Exception(f"In check_datasets, column '{CATEGORY_COLUMN}' " + f"not found in '{CSV_PATH}'")
        vprint("\t\t\t[+] CSV file", verbose)

        datasets_info[dataset] = (CATEGORY_COLUMN, IMAGE_COLUMN, IMAGES_PATH, CSV_PATH)
        vprint(f"\t\t[+] Dataset {i}\n", verbose)
...
def prepare_datasets_information(input_dir: pathlib.Path,
    ...
    split_file = info_dir / "meta_splits.txt"
