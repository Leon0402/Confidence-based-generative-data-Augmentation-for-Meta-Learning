...
class MyMetaLearner(MetaLearner):
        ...
        if self.should_train:
            ...
            for i, batch in enumerate(meta_train_generator(self.train_batches)):
                ...
                if self.ncc and 0 in self.running_lenght:
                    ...
                    X_train, y_train, X_test, y_test = self.batch_to_task(X, y, self.support_size)

                    # Compute loss
                    task_weights = [p.clone() for p in self.weights]
                    out, loss = self.compute_out_and_loss(self.meta_learner, task_weights, X_train, y_train, X_test,
                                                          y_test, True)
    ...
    def meta_valid(self, meta_valid_generator: Iterable[Any]) -> None:
        ...
        for task in meta_valid_generator(self.val_tasks):
            ...
            # Evaluate learner
            out, _ = self.compute_out_and_loss(self.val_learner, task_weights, X_train, y_train, X_test, y_test, False,
                                               True, num_ways)
    ...
    def update_prototypes(self,
        ...
        # Compute embeddings
        with context():
            current_embeddings = model.forward_weights(X, weights, embedding=True)
            if grad:
    ...
    def compute_out_and_loss(self,
        ...
        if perform_inner_step:
            ...
            for _ in range(self.T):
                # Compute gradients
                if self.ncc:
                    grads = self.get_grads_ncc(model, X_train, y_train, X_test, y_test, weights, self.second_order,
                                               retain_graph)
                else:
        ...
        with context():
            if self.ncc:
                if training:
                    run_prototypes, run_lenght = self.update_prototypes(model, X_train, y_train, weights, grad=True)
...
class MyLearner(Learner):
    ...
    def fit(self, support_set: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, int, int]) -> Predictor:
        ...
        if self.ncc:
            with torch.no_grad():
                prototypes = process_support_set(self.learner, task_weights, X_train, y_train, n_ways)
