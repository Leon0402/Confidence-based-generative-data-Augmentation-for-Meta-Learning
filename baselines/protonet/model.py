...
class MyMetaLearner(MetaLearner):
    ...
    def compute_out_and_loss(self,
        ...
        with context():
            prototypes = process_support_set(self.meta_learner, self.weights, X_train, y_train, num_classes)
            out = process_query_set(self.meta_learner, self.weights, X_test, prototypes)
            if no_loss:
                loss = None
            else:
                loss = self.meta_learner.criterion(out, y_test) / (num_classes * len(y_test))
...
class MyLearner(Learner):
    ...
    def fit(self, support_set: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, int, int]) -> Predictor:
        ...
        with torch.no_grad():
            prototypes = process_support_set(self.learner, self.weights, X_train, y_train, n_ways)
