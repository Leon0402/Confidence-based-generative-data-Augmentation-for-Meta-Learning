...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Conv2d(in_channels=3, kernel_size=7, out_channels=64, stride=2, padding=3, bias=False)
        ...
        self.model.update({"out": nn.Linear(in_features=self.in_features, out_features=self.num_classes).to(dev)})
    ...
    def forward_weights(self, x: torch.Tensor, weights: List[torch.Tensor], embedding: bool = False) -> torch.Tensor:
    ...
    def modify_out_layer(self, num_classes: int) -> None:
        ...
        Args:
            num_classes (int): Number of classes that the model should predict.
        """
        self.model.out = nn.Linear(in_features=self.in_features, out_features=num_classes).to(self.dev)
